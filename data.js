...
function loadDataFromStorage() {
    ...
    if (savedData) {
        window.tpsData = JSON.parse(savedData);
        window.nextId = savedNextId ? parseInt(savedNextId) : Math.max(...window.tpsData.map(t => t.id), 0) + 1;
    } else {
        window.tpsData = [...defaultTpsData];
        window.nextId = 16;
        saveDataToStorage(false); // Save default data without password
    }
}
...
function saveDataToStorage(requirePassword = true) {
    return new Promise((resolve, reject) => {
        const doSave = () => {
            localStorage.setItem(STORAGE_KEY, JSON.stringify(window.tpsData));
            localStorage.setItem(NEXT_ID_KEY, window.nextId.toString());
            resolve(true);
        };
        ...
        // Minta password untuk menyimpan
        Swal.fire({
...
// Fungsi untuk menghapus semua data (reset ke default)
function resetToDefaultData() {
    Swal.fire({
        title: '⚠️ Reset Data',
        text: 'Apakah Anda yakin ingin mereset semua data TPS ke default? Perubahan akan hilang!',
        icon: 'warning',
        showCancelButton: true,
        ...
        if (result.isConfirmed) {
            ...
            saveDataToStorage(true).then(() => {
                ...
                // Jika password salah, reset batal
                Swal.fire('Gagal', 'Reset data dibatalkan karena password salah', 'error');
            });
        }
    });
}

// Fungsi untuk mengkonversi ke format GeoJSON (jika diperlukan)
function convertToGeoJSON() {
    const features = window.tpsData.map(tps => ({
        type: "Feature",
        geometry: {
            type: "Point",
...
// Ekspor data ke global window
window.tpsData = window.tpsData;
window.nextId = window.nextId;
window.TPA = TPA;
